...
主要错误产生在`getCommands()`函数，可能产生多种错误，用`errno`描述错误类型，返回非负代表正常解析无错误（数值为解析得到的指令个数）；返回-1代表解析过程中出错（可能是某一条指令可能是多条指令）。为了区分不同指令的错误，将错误码`errno`嵌入command对象中。
...
typedef struct command
{
    ...
    unsigned char pipe;					// 是否需要创建pipe，最后一个redirect对象为pipe的重定向
    // errno
    unsigned int errno;                 // 是否出错，以及出错码（0代表无错，1代表出错）
} Command;
```
